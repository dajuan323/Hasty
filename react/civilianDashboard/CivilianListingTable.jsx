...
function CivilianListingTable() {
  ...
  useEffect(() => {
    if (pageData.filter > 0) {
      ...
      listingService.getCurrent(pageData.pageIndex, pageData.pageSize).then(onGetPageSuccess).catch(onGetPageError);
    }
  ...
  const mapListing = (aListing, index) => {
    return <CivilianTableRod key={index} listing={aListing} updatePage={updatePage} deleteHandler={deleteHandler} />;
  };
    ...
    <div className=" height-lord ">
      <Card className="height-lord ">
        <Card.Body>
          ...
          <Link className=" btn btn-primary btn-size position-absolute top-0 end-0 m-3 me-5" to="/listing/create">
            Add Listing
          </Link>
          <Link className=" btn btn-primary btn-size position-absolute top-0 end-0 m-3" to="/listings/map">
            <FaMap />
          </Link>
...
export default CivilianListingTable;
